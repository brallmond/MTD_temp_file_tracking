...
from sca_controller import sca_com, sca_cont, sca_chip
...
if (args.reset == 1): sca_cont.send_command(ch, leng, com, data, args.sca_addr, args.reset)  

foo = sca_chip(path=args.connection, fpga=args.fpga, link=args.link)

#sca_chip = sca_chip(path=args.connection, fpga=args.fpga, link=args.link)

print("Try i2c write")
foo.init_i2c(0) #ENI2C0 bit enabled
#foo.init_i2c(1) #ENI2C1 bit enabled
# only those two channels are used in the CC
#foo.write_i2c(0, 0)
              #channel, nbytes, freq
foo.write_i2c(0, 2, "400kHz")
foo.read_i2c(0)

#foo.read_control_regs('CRB')

#print("Try SEU")
#time.sleep(2.5)
#foo.read_control_regs("SEU")
#print()

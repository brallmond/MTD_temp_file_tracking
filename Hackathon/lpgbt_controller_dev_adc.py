...
class lpgbt_chip(LpgbtV1):
    ...
    Functions below uses the analog input ports to read the temparatures. 

    """

    def getPt1000Temperature(resistance):
        """
        Resistance to temperature conversion function
        """
        for i in range(len(pt1000_resistance_array)):
   #        print(pt1000_resistance_array[i])
            if resistance >= pt1000_resistance_array[i] and resistance <= pt1000_resistance_array[i+1]:
                tempL = i-100
                tempH = tempL + 1
                RL = pt1000_resistance_array[i]
                RH = pt1000_resistance_array[i+1]
                #print ("RL ", pt1000_resistance_array[i])
                #print ("RH ", pt1000_resistance_array[i+1])
                #print ("tempL", i-100.0)
                #print ("tempH", i-100.0+1)
                #print(temp)
                temp = tempL + (resistance - RL)/(RH-RL)*(tempH-tempL)
            else:
                raise ValueError("Invalid resistance value, please check Pt1000 database")
                
    return temp

    
    def set_adc_calib(self, slope, offset, cur_DAC):
        """
        Function to set/change the adc calibration 
        """
        self.adc_calib_mapping["slope"] = slope
        self.adc_calib_mapping["offset"] = offset
        self.adc_calib_mapping["cur_DAC"] = cur_DAC
       

    # ADC setup and read
    def mk_default_current_mask(self):
        """
        Returns the default current mask (an 8-bit mask) for the ADC ports.
        """
       num = 0
    ...
    def set_adc(self):
        """
        Enable current DAC configuration
        """
       # Lpgbt 0 (address 0x70); default configuration = 0b10001111
       # Lpgbt 1 (address 0x71); default configuration = 0b11001111 
    ...
    def read_adc(self, thesensor):
        ...
        """
        Get adc_mapping to configure the ADC, perform conversion and return the adc values.
        """
    ...
    def read_temp(self, thesensor):

        """
        Then get the calibration to convert adc values to temperature. 
        """
        slope_ = self.adc_calib_mapping["slope"]
        offset_ = self.adc_calib_mapping["offset"]
        cur_DAC_ = self.adc_calib_mapping["cur_DAC"]

        adc_val = self.read_adc(thesensor)     
        resistance = (slope_*adc_val + offset_)/cur_DAC_
        temperature = getPt1000Temperature(resistance)
        print("{} temperature: {} deg C".format(thesensor, temperature))
        return temperature

...
class lpgbt_chip(LpgbtV1):
    ...
    def set_ecelink_port(self, ec_eport=28):
       """
          the lpGBT features one eLink which is special. It communicates through EC. operates at one fixed data rate of 80 Mbps. 
          in the transceiver mode, the EC channel could be used to control other lpGBT chips or GBT-SCA.
          The port number of this special link is arbitrary and depends on the CC / hardware configuration. In BTL CCv2.2 the
          dedicated ePort is the last one (eport 28).
          
          Arguments:
             ec_eport: The port number for the special port designed for EC communication. Default is 28
       """

       self.indECELINK = ec_eport



    def setup_ECLK(self, elinkid, freq="160"):

       """ 
           Set e-clock frequency for a single eport 
           Arguments:
               freq: a string in ["disabled", "40", "80", "160", "320", "640", "1280", "loopback"]. Default freq="160"
        """

       assert (isinstance(freq, str) and freq in self.eclk_freq_map.keys()), "Be sure that freq value is a string and its value is in disabled,40,80,160,320,640,1280,loopback"
       assert elinkid in range(28), "elink given value {} is more than max number of elinks 28".format(elinkid)
       
       self.eclk_setup(elinkid,freq=self.eclk_freq_map[freq])
    ...
    def setup_EPRX(self, dll_current=3, dll_confirm_count=3, coarse_lock_detection=False, data_gating_enable=False, fsm_clk_always_on=False, reinit_enable=True):

        self.eprx_general_config(dll_current=dll_current, dll_confirm_count=dll_confirm_count, coarse_lock_detection=coarse_lock_detection, data_gating_enable=data_gating_enable, fsm_clk_always_on=fsm_clk_always_on, reinit_enable=reinit_enable)
    ...
    def setup_ELINKS(self, ec_clk_freq="40", clk_freq="160"):
       """
       ...
       self.eclk_setup(self.indECELINK,freq=self.eclk_freq_map[ec_clk_freq])
       
       for eind in range(self.nELINKS):
            if eind == self.indECELINK: self.eclk_setup(self.indECELINK,freq=self.eclk_freq_map[ec_clk_freq])
            else: self.eclk_setup(eind,freq=self.eclk_freq_map[clk_freq])
    ...
    def mk_default_current_mask(self):
       num = 0
       for key in self.adc_mapping.keys():
          num |= self.adc_mapping[key]["cur_mask"] << self.adc_mapping[key]["adc_port"]
    ...
    def set_adc(self):
       ...
       print(self.mk_default_current_mask()) 
    ...
    def read_adc(self, thesensor):
       map_ = self.adc_mapping[thesensor]
    ...
    def mk_default_direction(self):
       num = 0
       for key in self.gpio_mapping.keys():
          num |= self.gpio_mapping[key]["current_dir"] << self.gpio_mapping[key]["gpio_port"]
    ...
    def set_gpio_direction(self):
       ...
       print("---> Setting direction to {}".format(self.mk_default_direction())) 
       self.gpio_set_dir(self.mk_default_direction())

    def read_gpio_output(self, thefunction):
        
       map_ = self.gpio_mapping[thefunction]
       bit_ = map_["gpio_port"]
       assert map_["current_dir"] == 1 , "The port {} for function {} is not in output state (dir = {})".format(bit_, thefunction, map_["current_dir"])
    ...
    def read_gpio_input(self, thefunction):
        ...
        assert map_["current_dir"] == 0, "The port {} for function {} is not in input state (dir = {})".format(bit_, thefunction, map_["current_dir"])
    ...
    def set_gpio_bit_direction(self, thefunction, dir_=0):
        ...
        assert map_["current_dir"] == 1, "The port {} is an input port (dir = {}). The direction can be changed only for pins of output type)".format(bit_, map_["current_dir"])
        
        self.gpio_set_dir_bit(bit_, dir_)

    def get_gpio_bit_direction(self, thefunction):
    
        return self.gpio_get_dir_bit(self.gpio_mapping[thefunction]["gpio_port"])
    ...
    # key: MHz eclk frequency, value: value to be written in register
    eclk_freq_map = {
       "disabled": 0, #disabled
       "40": 1,
       "80": 2,
